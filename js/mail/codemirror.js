...
var sel_dontScroll={scroll:!1};var sel_mouse={origin:"*mouse"};var sel_move={origin:"+move"}
...
return out}
var sawReadOnlySpans=!1;var sawCollapsedSpans=!1
function seeReadOnlySpans(){sawReadOnlySpans=!0}
...
if(a==linedir){return!0}
if(b==linedir){return!1}
return a<b}
var bidiOther=null
...
this.from=from;this.to=to}
return function(str){if(!bidiRE.test(str)){return!1}
...
return order}
var noHandlers=[]
var on=function(emitter,type,f){if(emitter.addEventListener){emitter.addEventListener(type,f,!1)}else if(emitter.attachEvent){emitter.attachEvent("on"+type,f)}else{var map$$1=emitter._handlers||(emitter._handlers={})
map$$1[type]=(map$$1[type]||noHandlers).concat(f)}}
function getHandlers(emitter,type){return emitter._handlers&&emitter._handlers[type]||noHandlers}
function off(emitter,type,f){if(emitter.removeEventListener){emitter.removeEventListener(type,f,!1)}else if(emitter.detachEvent){emitter.detachEvent("on"+type,f)}else{var map$$1=emitter._handlers,arr=map$$1&&map$$1[type]
...
if(found>-1){this.pos=found;return!0}},backUp:function(n){this.pos-=n},column:function(){if(this.lastColumnPos<this.start){this.lastColumnValue=countColumn(this.string,this.start,this.tabSize,this.lastColumnPos,this.lastColumnValue)
...
runMode(cm,line.text,overlay.mode,!0,function(end,style){var start=i
...
if(asArray){tokens.push(getObj(!0))}}
...
var builder={pre:elt("pre",[content],"CodeMirror-line"),content:content,col:0,pos:0,cm:cm,trailingSpace:!1,splitSpaces:(ie||webkit)&&cm.getOption("lineWrapping")}
lineView.measure={}
...
right=!0}else if(ch==bidiRight(part)&&partPos<order.length-1&&part.level<order[partPos+1].level){part=order[++partPos]
...
if(y<0){return PosWithInfo(doc.first,0,!0,-1)}
...
var wrongLine=!1,adjust=2*cm.display.wrapper.clientWidth
...
if(innerOff<=charSize.bottom&&innerOff>=charSize.top&&Math.abs(x-charSize.right)<xDiff){outside=!1
ch++
...
for(var i=0;i<doc.sel.ranges.length;i++){if(primary===!1&&i==doc.sel.primIndex){continue}
...
if(coords.top+box.top<0){doScroll=!0}
else if(coords.bottom+box.top>(window.innerHeight||document.documentElement.clientHeight)){doScroll=!1}
...
var highlighted=highlightLine(cm,line,tooLong?copyState(doc.mode,state):state,!0)
...
Selection.prototype={primary:function(){return this.ranges[this.primIndex]},equals:function(other){var this$1=this;if(other==this){return!0}
...
if(cmp(here.anchor,there.anchor)!=0||cmp(here.head,there.head)!=0){return!1}}
return!0},deepCopy:function(){var this$1=this;var out=[]
...
return!1},contains:function(pos,end){var this$1=this;if(!end){end=pos}
...
TextMarker.prototype.changed=function(){var pos=this.find(-1,!0),widget=this,cm=this.doc.cm
if(!pos||!cm){return}
...
return!0})}),clearGutter:docMethodOp(function(gutterID){var this$1=this;var i=this.first
...
function onKeyPress$1(e){var cm=this
...
e_preventDefault(e)
break
case 3:if(captureRightClick){onContextMenu$1(cm,e)}
...
var dragEnd=operation(cm,function(e2){if(webkit){display.scroller.draggable=!1}
...
else{ourRange=word}}else if(type=="triple"){var line=new Range(Pos(start.line,0),clipPos(doc,Pos(start.line+1,0)))
if(cm.display.shift||doc.extend)
...
setSelection(doc,normalizeSelection(ranges.concat([ourRange]),ourIndex),{scroll:!1,origin:"*mouse"})}else if(ranges.length>1&&ranges[ourIndex].empty()&&type=="single"&&!e.shiftKey){setSelection(doc,normalizeSelection(ranges.slice(0,ourIndex).concat(ranges.slice(ourIndex+1)),0),{scroll:!1,origin:"*mouse"})
...
var cur=posFromMouse(cm,e,!0,type=="rect")
if(!cur){return}
...
function onContextMenu$1(cm,e){if(eventInWidget(cm.display,e)||contextMenuInGutter(cm,e)){return}
...
signal(this,"optionChange",this,option)},getOption:function(option){return this.options[option]},getDoc:function(){return this.doc},addKeyMap:function(map$$1,bottom){this.state.keyMaps[bottom?"push":"unshift"](getKeyMap(map$$1))},removeKeyMap:function(map$$1){var maps=this.state.keyMaps
...
{replaceOneSelection(this$1.doc,i,new Range(from,newRanges[i].to()),sel_dontScroll)}}else if(range$$1.head.line>end){indentLine(this$1,range$$1.head.line,how,!0)
...
return intoCoordSystem(this,lineObj,{top:0,left:0},mode||"page",includeWidgets).top+(end?this.doc.height-heightAtLine(lineObj):0)},defaultTextHeight:function(){return textHeight(this.display)},defaultCharWidth:function(){return charWidth(this.display)},getViewport:function(){return{from:this.display.viewFrom,to:this.display.viewTo}},addWidget:function(pos,node,scroll,vert,horiz){var display=this.display
...
if(scroll)
{scrollIntoView(this,left,top,left+node.offsetWidth,top+node.offsetHeight)}},triggerOnKeyDown:methodOp(onKeyDown),triggerOnKeyPress:methodOp(onKeyPress$1),triggerOnKeyUp:onKeyUp,execCommand:function(cmd){if(commands.hasOwnProperty(cmd))
...
else{return dir<0?range$$1.from():range$$1.to()}},sel_move)}),deleteH:methodOp(function(dir,unit){var sel=this.doc.sel,doc=this.doc
if(sel.somethingSelected())
{doc.replaceSelection("",null,"+delete")}
else{deleteNearSelection(this,function(range$$1){var other=findPosH(doc,range$$1.head,dir,unit,!1)
...
function moveOnce(boundToLine){var next=(visually?moveVisually:moveLogically)(lineObj,ch,dir,!0)
if(next==null){if(!boundToLine&&findNextLine()){if(visually){ch=(dir<0?lineRight:lineLeft)(lineObj)}
else{ch=dir<0?lineObj.text.length:0}}else{return!1}}else{ch=next}
return!0}
...
if(!cmp(pos,result)){result.hitSide=!0}
return result}
...
on(div,"cut",onCopyCut)};ContentEditableInput.prototype.prepareSelection=function prepareSelection$1(){var result=prepareSelection(this.cm,!1)
...
{this$1.cm.operation(function(){return this$1.cm.curOp.selectionChanged=!0})}},20)};ContentEditableInput.prototype.showMultipleSelections=function showMultipleSelections(info){removeChildrenAndAdd(this.cm.display.cursorDiv,info.cursors)
...
if(anchor.bad||head.bad){cm.curOp.selectionChanged=!0}})}}};ContentEditableInput.prototype.pollContent=function pollContent(){if(this.readDOMTimeout!=null){clearTimeout(this.readDOMTimeout)
...
return!0}};ContentEditableInput.prototype.ensurePolled=function ensurePolled(){this.forceCompositionEnd()};ContentEditableInput.prototype.reset=function reset(){this.forceCompositionEnd()};ContentEditableInput.prototype.forceCompositionEnd=function forceCompositionEnd(){if(!this.composing){return}
...
{runInOp(this$1.cm,function(){return regChange(this$1.cm)})}},80)};ContentEditableInput.prototype.setUneditable=function setUneditable(node){node.contentEditable="false"};ContentEditableInput.prototype.onKeyPress=function onKeyPress(e){e.preventDefault()
...
{closing=!0}}else if(node.nodeType==3){var val=node.nodeValue
if(!val){return}
...
if(this$1.cm.state.focused){this$1.slowPoll()}})};TextareaInput.prototype.fastPoll=function fastPoll(){var missed=!1,input=this
input.pollingFast=!0
...
return!0};TextareaInput.prototype.ensurePolled=function ensurePolled(){if(this.pollingFast&&this.poll()){this.pollingFast=!1}};TextareaInput.prototype.onKeyPress=function onKeyPress(){if(ie&&ie_version>=9){this.hasSelection=null}
...
on(window,"mouseup",mouseup)}else{setTimeout(rehide,50)}};TextareaInput.prototype.readOnlyChanged=function readOnlyChanged(val){if(!val){this.reset()}};TextareaInput.prototype.setUneditable=function setUneditable(){};TextareaInput.prototype.needsContentAttribute=!1
