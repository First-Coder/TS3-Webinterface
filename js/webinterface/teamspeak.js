...
*/
	...
	{
		var instanz,
			message 					=	$('#instanzMessagePokeContent').val(),
			mode						=	$('#instanzMode').hasClass("active");
		
		if(document.getElementById('selectInstanzMsgPoke'))
		{
			instanz						=	$("#selectInstanzMsgPoke").val()
		}
		else
		{
			instanz						=	$("input[name='instanzMsgPoke']:checked").val()
		};
		...
		if(message != '')
		{
			$.ajax({
				...
				success: function(data)
				{
					if(data == "done")
					{
						setNotifySuccess(lang.server_msg_poke_done);
					}
					else
					{
...
*/
	function createChannel()
	{
		...
		channeldata.cpid												=		$('#cpid').val();
		channeldata.channel_name										=		$('#channel_name').val();
		channeldata.channel_topic										=		$('#channel_topic').val();
		channeldata.channel_description									=		$('#channel_description').val();
		channeldata.channel_codec										=		$('#channel_codec').val();
		channeldata.channel_codec_quality								=		$('#channel_codec_quality').val();
		channeldata.channel_maxclients									=		$('#channel_maxclients').val();
		channeldata.channel_maxfamilyclients							=		$('#channel_maxfamilyclients').val();
		channeldata.channel_flag_maxfamilyclients_inherited				=		$('#channel_flag_maxfamilyclients_inherited').val();
		channeldata.channel_needed_talk_power							=		$('#channel_needed_talk_power').val();
		channeldata.channel_name_phonetic								=		$('#channel_name_phonetic').val();
		
		switch($('#channel_typ').val())
		{
			case "1":
		...
		$.ajax({
			...
			data: {
				...
				instanz:		instanz,
				port:			port,
				channeldata:	encodeURIComponent(JSON.stringify(channeldata))
			},
			success: function(data){
				if(data == 'done')
				{
					setNotifySuccess(lang.channel_created);
				}
				else
				{
					setNotifyFailed(data);
		...
		$.ajax({
			...
			success: function(data)
			{
				if(data == "done")
				{
					...
					setNotifySuccess(lang.token_successful_deleted);
				}
				else
				{
					setNotifyFailed(data);
...
*/
	...
	{
		var failState		=	false,
			tokenAnzahl		=	$('#tokenChooseAnzahl').val(),
			tokenDesc		=	escapeText($('#tokenChooseDescription').val()),
			tokenKindGroup	=	$('#tokenChooseKindOfGroup').val(),
			tokenGroup		=	$('#tokenChooseGroup').val(),
			tokenChannel	=	0;
				...
				success: function(data)
				{
					...
					if(informations['done'] == 'true')
					{
						...
						for(var k in informations)
						{
							...
							{
								newRow.push({
									groupname: $('#tokenChooseGroup :selected').text(),
									channel: (tokenChannel != 0) ? $('#tokenChooseChannel :selected').text() : "-",
									create_on: 'Now',
									token: informations[k]['token'],
									description: escapeText(informations[k]['description']),
									actions: '<button style="margin-bottom: 0px;padding: .0rem .75rem;" class="btn btn-sm btn-danger" onClick="deleteToken(\''+informations[k]['token']+'\')"><i class="fa fa-fw fa-trash"></i> '+lang.delete+'</button>'
								});
							};
						};
						
						tokenTable.bootstrapTable('prepend', newRow);
					}
					else
					{
	...
	function resetServer()
	{
		$.ajax({
			...
			success: function(data)
			{
				if(data == "")
				{
					setNotifyFailed("There was something wrong in the reset progress :/");
				}
				else
				{
					setNotifySuccess(lang.reset_server_success+"<br/>"+lang.server_token+": <i>"+data+"</i>");
				};
	...
	function createBackup()
	{
		...
		$.ajax({
			...
			data: {
				...
				instanz:		instanz,
				port:			port,
				kind:			(backupKind) ? 'channel' : 'server',
				kindChannel:	(backupChannelKind) ? "name" : "all"
			},
			success: function(data)
			{
				...
				{
					var newRow				=	[],
						splitData			=	data.replace(".json", "").split("_"),
						backupTable,
						folder;
					...
					if(backupKind)
					{
						...
						{
							backupTable		=	'channelBackupNameTable';
							folder			=	'channelname';
						}
						else
						{
							backupTable		=	'channelBackupNameSettingsTable';
							folder			=	'channelnamesettings';
						};
					}
					else
					{
						backupTable			=	'serverBackupTable';
						folder				=	'server';
					};
					...
					newRow.push({
						instanz: splitData[1],
						port: splitData[3],
						date: splitData[7]+":"+splitData[8]+"-"+splitData[6]+"."+splitData[5]+"."+splitData[4],
						actions: "<button class=\"btn btn-success btn-sm\" onClick=\"activateBackup('"+data+"', '"+folder+"');\"><i class=\"fa fa-check\"></i> <font class=\"hidden-md-down\">"+lang.activate+"</font></button>\
								<a href=\"./php/functions/functionsDownload.php?action=downloadBackup&port="+port+"&instanz="+instanz+"&type="+folder+"&name="+data+"\">\
									<button class=\"btn btn-primary btn-sm\"><i class=\"fa fa-download\"></i> <font class=\"hidden-md-down\">"+lang.download+"</font></button>\
								</a>\
								<button class=\"btn btn-danger btn-sm\" onClick=\"AreYouSure('"+lang.delete_backup+"', 'deleteBackup(\\'"+folder+"/"+data+"\\', \\'"+backupTable+"\\', \\'"+data.replace(".json", "")+"\\');');\"><i class=\"fa fa-trash\"></i> <font class=\"hidden-md-down\">"+lang.delete+"</font></button>",
						id: data.replace(".json", "")
					});
					
					$('#'+backupTable).bootstrapTable('prepend', newRow);
					setNotifySuccess(lang.backup_created);
				}
				else
				{
					setNotifyFailed(data);
...
*/
	function deleteBackup(path, id, removeId)
	{
		$.ajax({
			...
			success: function(data){
				...
				{
					setNotifyFailed(lang.file_could_not_deleted);
				};
		...
		$.ajax({
			...
			success: function(data)
			{
				if(data == "done")
				{
					...
					setNotifySuccess(lang.icon_successful_deleted);
				}
				else
				{
					setNotifyFailed(lang.file_could_not_deleted);
...
*/
	function addBan()
	{
		var input		=	escapeText($('#banInput').val()),
			banReason	=	escapeText($('#banReason').val()),
			banTime		=	$('#banTime').val(),
			bantype		=	"uid";
		...
		if(input != "" && banTime != "")
		{
			$.ajax({
				...
				url: "./php/functions/functionsTeamspeakPost.php",
				data: {
					action:		'clientBanManuell',
					port:		port,
					bantype:	bantype,
					instanz:	instanz,
					input:		encodeURIComponent(input),
					time:		banTime,
					reason:		encodeURIComponent(banReason)
				},
				success: function(data)
				{
					if(data == "done")
					{
						setNotifySuccess(lang.ban_successfull_added);
						teamspeakBansInit();
					}
					else
					{
		...
		{
			setNotifyFailed(lang.add_ban_error);
		};
	};
		...
		$.ajax({
			...
			success: function(data)
			{
				if(data == "done")
				{
					...
					setNotifySuccess(lang.ban_successful_deleted);
				}
				else
				{
					setNotifyFailed(data);
...
*/
	function createServer(requestName = "", requestPw = "", filename, isRequest = false)
	{
		...
		serverdata.virtualserver_log_client								=	(!isRequest) ? $('#serverCreateProtokolClient').val() : ts3_server_create_default['virtualserver_log_client'];
		serverdata.virtualserver_log_query								=	(!isRequest) ? $('#serverCreateProtokolQuery').val() : ts3_server_create_default['virtualserver_log_query'];
		serverdata.virtualserver_log_channel							=	(!isRequest) ? $('#serverCreateProtokolChannel').val() : ts3_server_create_default['virtualserver_log_channel'];
		serverdata.virtualserver_log_permissions						=	(!isRequest) ? $('#serverCreateProtokolRights').val() : ts3_server_create_default['virtualserver_log_permissions'];
		serverdata.virtualserver_log_server								=	(!isRequest) ? $('#serverCreateProtokolServer').val() : ts3_server_create_default['virtualserver_log_server'];
		serverdata.virtualserver_log_filetransfer						=	(!isRequest) ? $('#serverCreateProtokolTransfer').val() : ts3_server_create_default['virtualserver_log_filetransfer'];
			...
			success: function(data){
				...
				if(informations.success === "1")
				{
					$.notify({
						...
						message: lang.server_id+":<i> "+informations['serverid']+"</i><br />"+lang.port+":<i> "+informations['port']+"</i><br />"+lang.token+":<i> "+informations['token']+"</i>",
						icon: 'fa fa-check'
					},{
						type: 'info',
						allow_dismiss: true,
...
*/
	function showServerRequest(file)
	{
		...
		{
			$("#mainContent").load('./php/teamspeak/web_teamspeak_server_requests_info.php', { "file" : file }, function()
			{
