...
*/
	...
	{
		if($('#ticketBetreff').val() == '' || $('#ticketMessage').val() == '')
		{
	...
	function deleteTicket(id)
	{
		$.ajax({
			...
			success: function(data){
				if(data == 'done')
				{
					setNotifySuccess(lang.ticket_deleted);
	...
	function closeTicket(id)
	{
		$.ajax({
			...
			success: function(data){
				if(data == 'done')
				{
					setNotifySuccess(lang.ticket_close);
					...
					if(document.getElementById("deleteTicketSection"+id))
					{
						$('#deleteTicketSection'+id).show();
					};
					
					document.getElementById("status"+id).innerHTML = '<i class="fa fa-close"></i> '+lang.closed;
				}
				else
				{
					setNotifyFailed(data);
		...
		else
		{
			$.ajax({
				...
				success: function(data){
					if(data == 'done')
					{
						setNotifySuccess(lang.ticket_answer);
						ticketInit();
					}
					else
					{
