...
$(function(){
	$.ajax({
		...
		success: function(data)
		{
			...
			while(!subChannelsDone && document.getElementById('tree'))
			{
				tmp					=	0;
...
function delSubSubChannel(id)
{
	...
	var tmp2					=	"0";
	var checkDone				=	false;
	while(!checkDone && document.getElementById('tree'))
	{
		checkDone				=	true;
		...
		else
		{
			...
			if(typeof($("#channel_"+lastCid).next()) != "undefined")
			{
				if($("#channel_"+lastCid).next().attr('channelpid') != 0 && $("#channel_"+lastCid).next().attr("id") != "tree_content" && update)
				{
					...
					while(checkPid != 0 && document.getElementById('tree'))
					{
...
function setSubChannel(channel, wantedSubs, delTimer, update = false, lastCid = -1)
{
	...
	else
	{
		...
		inhalt											+=		'<div class="col-md-12">';
		inhalt											+=			'<div class="col-md-7 col-xs-7" style="padding-left: '+(2+(tabs*3))+'%;">';
...
function setClient(client, delTimer)
{
	...
	inhalt											+=		'<div class="col-md-12">';
	inhalt											+=			'<div class="col-md-7 col-xs-7" style="padding-left: '+(5+($('#channel_'+client.nick_cid).attr('tabs')*3))+'%;">';
